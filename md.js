...
sock.sendMessage(from, { caption: anj, location: { jpegThumbnail: ktedh }, templateButtons: button, footer: `Created By BloodSword`, mentions: [m.sender] })
.catch ((err) => reply(err))
}
...
sock.sendList(m.chat, `Pilih 1 Setallmenu Di Bawah`, `Creator BloodSword𓁄⁩`, `Hello Owner !!!`, `SELECT`, sections, troli)
}
}
addCmd(command.slice(0), 1, commund)
break
...
url: "https://www.whatsapp.com/otp/copy/https://chat.whatsapp.com/kepoanjing",
}
},
{ 
quickReplyButton: { 
...
sock.send5ButImg(m.chat, memegnu, `Creator BloodSword𓁄⁩`, ppnyauser, btn)
} else if (setbot.templateGif) {
sock.send5ButGif(m.chat, memegnu, `Creator BloodSword𓁄⁩`, tytyd, btn)
} else if (setbot.templateVideo) {
sock.send5ButVid(m.chat, memegnu, `Creator BloodSword𓁄⁩`, global.vidkir, btn)
} else if (setbot.templateMsg) {
sock.send5ButMsg(m.chat, memegnu, `Creator BloodSword𓁄⁩`, btn)
} else if (setbot.templateLoc) {
sock.send5ButLoc(m.chat, memegnu, `Creator BloodSword𓁄⁩`, tod, btn)
}
}
addCmd(command.slice(0), 1, commund)
break
...
if (isBan) return reply(api.ban)
reply(api.wait)
let eek = await fetchJson(`https://bx-hunter.herokuapp.com/api/igdownload?url=${text}&apikey=FuckBitch`)
sock.sendMessage(m.chat, { video: { url: eek.linkdownload } }, { quoted: m })
}
break
case 'igfoto':{
if (!text) throw 'Link Nya?'
reply(api.wait)
eek = await getBuffer(`https://leyscoders-api.herokuapp.com/api/instagram/photo?url=${text}&apikey=IkyOgiwara`)
pll = `Nih Kak シ︎`
let message = await prepareWAMessageMedia({ image: eek }, { upload: sock.waUploadToServer })
...
await sock.sendButtonText(m.chat, buttonns, `Mode Antionce`, `_Creator BloodSword𓁄⁩_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'antilink':
...
await sock.sendButtonText(m.chat, buttons, `Mode Antilink`, `_Creator BloodSword𓁄⁩_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'tagall': {
